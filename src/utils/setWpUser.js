...
export default async function setWpUser(auth, initData) {
	// Загрузка существующего пользователя
	const user = initData.user
	const username = `user${user.id}`
	let wpId = await getWpUserId(auth, username)
	if (wpId) return wpId
	...
	wpId = await regWpUser(auth, username, email, password, nickname, startParam, user.language_code)
	return wpId
}